...
def mock_deps(tmp_path):
    ...
    db = MagicMock()
    return config, downloader, detector, compositor, concatenator, db


class TestPipeline:
    def test_dry_run(self, mock_deps):
        config, dl, det, comp, concat, db = mock_deps
        pipeline = Pipeline(config, dry_run=True, downloader=dl, detector=det,
                          compositor=comp, concatenator=concat, db=db)
    ...
    def test_rebuild_outputs(self, mock_deps, tmp_path):
        config, dl, det, comp, concat, db = mock_deps
        db.clips.get_included_detected_clips.return_value = [
            {"detection_image": str(tmp_path / "img1.png"), "detected_video": str(tmp_path / "v1.mp4")},
        ]
        comp.composite.return_value = tmp_path / "comp.jpg"
        concat.concatenate.return_value = tmp_path / "vid.mp4"
    ...
    def test_rebuild_without_db_raises(self, mock_deps):
        config, dl, det, comp, concat, _ = mock_deps
        pipeline = Pipeline(config, downloader=dl, detector=det,
                          compositor=comp, concatenator=concat, db=None)
